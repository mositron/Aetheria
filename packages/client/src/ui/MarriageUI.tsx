...
export function MarriageUI({ room }: { room: Room<WorldState> }) {
  ...
  return (
    <>
      ...
      >
        <FocusTrap focusTrapOptions={{ allowOutsideClick: true }}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={t("marriage.title")}
            onClick={e => e.stopPropagation()}
          >
            <GameFrame title={t("marriage.title")} variant="violet" className="w-80 max-w-[94vw]">
              <button
                onClick={() => setOpen(false)}
                ...
                <div className="space-y-3 p-3">
                  <div className="text-center">
                    ...
                    <div className="text-cyan-300 text-xs mt-1">
                      {t("marriage.marriedOn", { date: me?.marriageDate ? new Date(me.marriageDate).toLocaleDateString("th-TH") : "N/A" })}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1 text-center p-2 rounded-lg bg-violet-900/30 border border-violet-400/30">
                      ...
                      <div className="text-slate-400 text-[10px]">{t("marriage.spouse")}</div>
                  ...
                  >
                    {t("marriage.divorceBtn")}
                  </button>
                </div>
              ) : (
                <div className="space-y-3 p-3">
                  <div className="text-center text-slate-300 text-sm">{t("marriage.notMarried")}</div>
                  <input
                    value={targetName}
                    onChange={e => setTargetName(e.target.value)}
                    placeholder={t("marriage.spousePlaceholder")}
      ...
      <ConfirmDialog
        open={confirmDivorce}
        title={t("marriage.divorceConfirmTitle")}
        message={t("marriage.divorceConfirmMsg")}
        confirmLabel={t("marriage.divorceConfirmBtn")}
        severity="danger"
