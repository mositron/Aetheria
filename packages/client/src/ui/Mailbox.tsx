  ...
  return (
    <div data-no-screen-joy role="dialog" aria-modal="true" className="absolute inset-0 z-40 flex items-center justify-center bg-black/65 backdrop-blur-sm py-16 px-4" onClick={() => setOpen(false)}>
      <div className="w-[26rem] max-w-[94vw] flex flex-col min-h-0" style={{ maxHeight: "calc(100vh - 8rem)" }} onClick={(e) => e.stopPropagation()}>
        <GameFrame
          title={t("mail.title") + (unread > 0 ? ` · ${unread} ${t("mail.newMailCount", { count: unread })}` : "")}
        ...
        >
          ...
          <div className="flex gap-1 mb-2 flex-shrink-0">
            <button onClick={() => setMode("inbox")} className={`flex-1 py-1.5 text-xs font-bold rounded-lg border-2 ${mode === "inbox" ? "border-cyan-300 bg-cyan-500/30 text-white" : "border-slate-700 bg-slate-900/60 text-slate-300"}`}>📥 {t("mail.inbox")}</button>
            <button onClick={() => setMode("send")} className={`flex-1 py-1.5 text-xs font-bold rounded-lg border-2 ${mode === "send" ? "border-cyan-300 bg-cyan-500/30 text-white" : "border-slate-700 bg-slate-900/60 text-slate-300"}`}>✏ {t("mail.send")}</button>
          </div>

          {mode === "inbox" ? (
            <div className="overflow-y-auto game-scroll flex-1 pr-1 space-y-1.5">
              {mails.length === 0 && <div className="text-center text-slate-400 py-6 text-sm">{t("mail.noMail")}</div>}
              {mails.map((m) => (
    ...
    <div className={`border-2 rounded-xl p-2 ${mail.read ? "border-slate-700 bg-slate-900/40" : "border-cyan-400 bg-cyan-500/15"}`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] text-slate-400 truncate">{t("mail.from")} {mail.fromName}</div>
        </div>
        {!mail.read && <span className="text-[9px] bg-rose-500 px-1.5 py-0.5 rounded text-white font-bold">{t("mail.newBadge")}</span>}
      </div>
      {expanded && (
        <div className="mt-2 pt-2 border-t border-slate-600/50 space-y-2">
          ...
          {hasGift && !mail.claimed && (
            <div className="bg-amber-900/30 border border-amber-400/40 rounded-lg p-2">
              <div className="text-[10px] text-amber-300 font-bold mb-1">{t("mail.giftLabel")}</div>
              ...
              >{t("mail.claimBtn")}</button>
            </div>
          )}
          {mail.claimed && <div className="text-[10px] text-emerald-300">{t("mail.claimed")}</div>}
        </div>
      )}
    </div>
  );
...
function ComposeMail({ room, me, onSent }: { room: Room<WorldState>; me: Player; onSent: () => void }) {
  ...
  return (
    <div className="overflow-y-auto game-scroll flex-1 pr-1 space-y-2">
      <div>
        <div className="text-[10px] text-cyan-300 uppercase font-bold mb-1">{t("mail.sendTo")}</div>
        <input value={to} onChange={(e) => setTo(e.target.value)} className="game-input" placeholder={t("mail.recipientPlaceholder")} />
      </div>
      <div>
        <div className="text-[10px] text-cyan-300 uppercase font-bold mb-1">{t("mail.subject")}</div>
        <input value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={60} className="game-input" placeholder={t("mail.subjectPlaceholder")} />
      </div>
      <div>
        <div className="text-[10px] text-cyan-300 uppercase font-bold mb-1">{t("mail.body")}</div>
        <textarea value={body} onChange={(e) => setBody(e.target.value)} maxLength={500} rows={3} className="game-input resize-none" placeholder={t("mail.messagePlaceholder")} />
      </div>
      <div>
        <div className="text-[10px] text-cyan-300 uppercase font-bold mb-1">{t("mail.attachMoney")}</div>
        ...
        <div className="text-[9px] text-slate-400">{t("mail.balance", { zeny: me.zeny })}</div>
      </div>
      <div>
        <div className="text-[10px] text-cyan-300 uppercase font-bold mb-1">{t("mail.attachItemLabel")}</div>
        <select value={itemIdx} onChange={(e) => setItemIdx(parseInt(e.target.value))} className="game-input">
          <option value={-1}>{t("mail.noItems")}</option>
          {items.map((s, i) => {
        ...
        {itemIdx >= 0 && (
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] text-slate-300">{t("mail.quantityLabel")}</span>
      ...
      <button onClick={send} className="btn-game w-full">{t("mail.sendBtnLabel")}</button>
    </div>
  );
}