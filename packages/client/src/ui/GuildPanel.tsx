  ...
  return (
    <div data-no-screen-joy className="absolute inset-0 z-40 flex items-center justify-center bg-black/65 backdrop-blur-sm py-12 px-4" onClick={() => setOpen(false)}>
      <div className="w-[24rem] max-w-[94vw]" onClick={(e) => e.stopPropagation()}>
        <GameFrame title={info ? `⚔ ${info.tag ? `[${info.tag}] ` : ""}${info.name}` : "⚔ กิลด์"}>
          ...
          {!info ? (
            <div className="space-y-3 pt-1">
              ...
              >
                <div className="text-[11px] text-cyan-300 font-bold">🛡 สร้างกิลด์ใหม่</div>
                <input value={createName} onChange={(e) => setCreateName(e.target.value)} maxLength={24} placeholder="ชื่อกิลด์ (สูงสุด 24 ตัว)" className="w-full bg-slate-900/80 border border-cyan-400/40 rounded px-2 py-1 text-xs text-white" />
                <input value={createTag} onChange={(e) => setCreateTag(e.target.value)} maxLength={4} placeholder="แท็ก (เช่น TGS — 4 ตัวอักษร)" className="w-full bg-slate-900/80 border border-cyan-400/40 rounded px-2 py-1 text-xs text-white uppercase" />
                <button type="submit" className="w-full bg-cyan-600 hover:bg-cyan-500 rounded px-3 py-1 text-xs font-bold text-white">สร้าง</button>
              </form>
              ...
              >
                <div className="text-[11px] text-violet-300 font-bold">🤝 เข้าร่วมกิลด์ที่มีอยู่</div>
                <input value={joinName} onChange={(e) => setJoinName(e.target.value)} maxLength={24} placeholder="ชื่อกิลด์ที่ต้องการเข้า" className="w-full bg-slate-900/80 border border-violet-400/40 rounded px-2 py-1 text-xs text-white" />
                <button type="submit" className="w-full bg-violet-600 hover:bg-violet-500 rounded px-3 py-1 text-xs font-bold text-white">เข้าร่วม</button>
              </form>
            </div>
          ) : (
            <div className="space-y-2 pt-1">
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-amber-300">หัวหน้า: <span className="font-bold">{info.leader}</span></span>
                <span className="text-slate-400">{info.members.length} คน</span>
              </div>
              ...
              <div className="bg-slate-900/60 border border-cyan-400/20 rounded p-1.5 space-y-1">
                <div className="text-[10px] text-cyan-300 font-bold">💬 แชทกิลด์</div>
                <div ref={scrollRef} className="max-h-28 overflow-y-auto game-scroll text-[10px] space-y-0.5">
                  {chat.length === 0 && <div className="text-slate-500 italic">เริ่มคุยกัน...</div>}
                  {chat.map((m, i) => (
                ...
                <form onSubmit={(e) => { e.preventDefault(); if (!chatText.trim()) return; room.send("guild:chat" as any, { text: chatText.trim() }); setChatText(""); }} className="flex gap-1">
                  <input value={chatText} onChange={(e) => setChatText(e.target.value)} placeholder="ส่งข้อความ..." maxLength={200} className="flex-1 bg-slate-900/80 border border-cyan-400/40 rounded px-2 py-1 text-xs text-white" />
                  <button type="submit" className="bg-cyan-600 hover:bg-cyan-500 rounded px-3 text-xs font-bold text-white">ส่ง</button>
                </form>
              </div>

              <button
                onClick={() => { if (confirm("ออกจากกิลด์?")) room.send("guild:leave" as any, {}); }}
