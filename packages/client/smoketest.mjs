...
async function main() {
  // 1) register
  const reg = await fetch(`${SERVER_HTTP}/auth/register`, {
    method: "POST",
  ...
  const room = await client.joinOrCreate("map_field", { token: reg.token, mapId: "field" });
  console.log("[joined]", { sessionId: room.sessionId, mapId: room.state.mapId });

  // 3) wait for state to populate
