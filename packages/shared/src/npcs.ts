  ...
  },
{
  ...
  },
  {
    ...
    color: "#7c3aed",
    icon: "🗺",
    dialog: "เดินตรงไปทางทิศต่างๆ จะเจอถ้ำที่เดินเข้าได้เลย — ไม่ต้องผ่าน portal!",
    choices: [
      { text: "🕳 ถ้ำเงา (NW, Lv 10+)",       action: { kind: "system", text: "ทิศตะวันตกเฉียงเหนือ พิกัด (-80, -60) — มี wolves + orcs" } },
      { text: "❄ ถ้ำน้ำแข็ง (NW, Lv 20+)",    action: { kind: "system", text: "เหนือถ้ำเงาขึ้นไป พิกัด (-75, -80) — yeti + ice wraith + snowman giant" } },
      { text: "🏜 ถ้ำทะเลทราย (NW, Lv 25+)",   action: { kind: "system", text: "พิกัด (-55, -85) — sand worm + scorpion lord" } },
      { text: "🌿 ถ้ำบึงเลน (S, Lv 30+)",      action: { kind: "system", text: "ทิศใต้ พิกัด (0, 85) — bog witch + swamp serpent" } },
      { text: "🌲 ถ้ำป่าลึก (NE, Lv 20+)",     action: { kind: "system", text: "พิกัด (78, -60) — รังออร์คขนาดใหญ่" } },
      { text: "⚜ ถ้ำผู้พิทักษ์ (N, Lv 40+)",   action: { kind: "system", text: "ทิศเหนือไกลสุด พิกัด (0, -90) — Dark Lord รออยู่!" }, requires: { minLevel: 30 } },
    ],
  },
];
